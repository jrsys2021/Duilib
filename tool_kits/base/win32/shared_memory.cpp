...
bool SharedMemory::MappedView::WriteData(const char* data)
{
	if (view_ == NULL)
		return false;

	strcpy_s((char*)view_, 64,data);
	return true;
}
bool SharedMemory::MappedView::OpenView(SharedMemory *shared_memory, bool read_write)
{
	...
	view_ = reinterpret_cast<unsigned char*>(::MapViewOfFile(shared_memory->mapping_, access, 0, 0, 64));
	return view_ != NULL;
}
